...
# --- 1. DIRECTORY CHECK ---
if [ ! -f "servarr/docker-compose.yml" ]; then
    echo -e "${RED}ERROR:${NC} Could not find 'servarr/docker-compose.yml'."
...
# Select inherently waits for ENTER
...
echo "1) OpenVPN"
echo "2) WireGuard"
# Changed: Now waits for user to type '1' or '2' and press ENTER
...
if [[ "$PROTO_SELECTION" == "2" ]]; then
    ...
    # Removing -s flag so user can see what they type to verify correctness
...
# --- 4. APPLY PERMISSIONS ---
# Since folders exist via git clone, we just ensure the user owns them.
...
echo -e "${GREEN}cd servarr && docker compose up -d${NC}"
echo ""